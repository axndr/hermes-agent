...
def _maybe_wrap_anthropic(
    ...
    try:
        ...
        if _safe_isinstance(client_obj, ClaudeACPClient):
            return client_obj
    except ImportError:
        pass
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from agent.claude_acp_client import ClaudeACPClient
        if isinstance(sync_client, ClaudeACPClient):
            return sync_client, model
    except ImportError:
        pass
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider in {"copilot-acp", "claude-acp"}:
            ...
            if provider == "claude-acp":
                ...
                # No `agent=` here — this is the auxiliary/side-call router
                # (MOA, fallback, verification calls), not the primary
                # conversation client, so there is no AIAgent session to
                # hang persistence off of. The returned ClaudeACPClient
                # still reuses ONE process across multiple .create() calls
                # made on this same instance (private session), it just
                # doesn't survive past this instance's lifetime.

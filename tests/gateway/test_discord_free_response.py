...
async def test_discord_free_response_channel_still_auto_threads(adapter, monkeypatch):
    """Free-response bypasses @mention gating, but not auto-threading."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    monkeypatch.setenv("DISCORD_FREE_RESPONSE_CHANNELS", "789")
    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)  # default true

    parent = FakeTextChannel(channel_id=789)
    fake_thread = FakeThread(channel_id=990, name="auto-thread", parent=parent)
    ...
    assert event.source.chat_id == "990"
